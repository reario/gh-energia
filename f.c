...
#ifdef DEBUG 
  printf("----------\n");
  printbitssimple(in[num_reg_in]); // 0
  printbitssimple(out[num_reg_out]); // 0
  printf("----------\n");
#endif
  if (what_in >=0) {
...
#ifdef DEBUG
  printf("----------\n");
  printbitssimple(in[addr_reg_in]);
  printbitssimple(out[addr_reg_out]);
  printf("----------\n");
#endif
   if (what_in >=0) {
...
int main (int argc, char ** argv) {
  ...
  while ( (ch = getopt (argc, argv, "ars")) != -1) {
    switch (ch) {
    case 'r':
      rflag=1;
      break;
  ...
  if (rflag == 1) {
    if (operate(mb_otb, // mb
		...
		FARI_ESTERNI_IN_SOPRA, // what_in numero bit all'interno del registro INGRESSI (se <0 non check gli ingressi)
