...
# drop schema public
#/usr/bin/psql -U postgres -d reario_db -h linux6 -p 5432 -c "drop schema public cascade"

# recreate schema public
#/usr/bin/psql -U postgres -d reario_db -h linux6 -p 5432 -c "create schema public AUTHORIZATION reario"
#/usr/bin/psql -U postgres -d reario_db -h linux6 -p 5432 -c "create extension tablefunc with schema public"

/usr/bin/pg_dump -s --clean --if-exists -h localhost -d reario_db --role=reario | /usr/bin/psql -U postgres -d reario_db -h linux6 -p 5432
